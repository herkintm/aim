...
// Konfiguracja MockAPI
const MOCKAPI_URL = "https://6544c2ed5a0b4b04436cd6e0.mockapi.io/records";

// Funkcje do obsługi rekordów publicznych
function fetchGlobalRecords() {
  fetch(MOCKAPI_URL)
    .then(res => {
    ...
    .then(data => {
      if (Array.isArray(data)) {
        ...
        let html = 'TOP RECORDS:<br>';
        sortedData.forEach((rec, idx) => {
          html += `${idx+1}. <b>${rec.nick}</b> — ${rec.score}<br>`;
        });
        document.getElementById('listR').innerHTML = html;
      }
    })
    .catch(() => {
      document.getElementById('listR').innerHTML = 'Failed to load records.';
    });
}

function fetchAndShowPublicRecords() {
  fetch(MOCKAPI_URL)
    .then(res => {
      if (!res.ok) throw new Error("Network response was not ok");
      return res.json();
    ...
    .then(data => {
      if (Array.isArray(data)) {
        ...
        let html = '<b>PUBLIC RECORDS</b><br><br>';
        sortedData.forEach(rec => {
          html += `<span style="font-weight:bold;">${rec.nick}</span> - ${rec.score}<br>`;
          html += `<span style="font-size:15px;color:#ffe600;">${rec.date}</span><br><br>`;
        });
        document.getElementById('publicRecords').innerHTML = html;
      }
    })
    .catch(() => {
      document.getElementById('publicRecords').innerHTML = 'Failed to load public records.';
    });
}

function saveGlobalRecord(nick, score) {
  ...
  .then(res => {
    ...
    fetchAndShowPublicRecords();
    fetchGlobalRecords();
  })
  .catch(error => console.error("Save error:", error));
}

function trySaveGlobalRecord(score) {
  const nick = prompt('Congratulations! Enter your nickname to save your public record:');
  if (nick && nick.trim() !== "") {
    saveGlobalRecord(nick.trim(), score);
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // (tutaj wklej cały pozostały kod z twojego pliku js.js)
});

// Dodaję kontener na publiczne rekordy po prawej stronie
var publicRecordsDiv = document.createElement('div');
publicRecordsDiv.id = "publicRecords";
publicRecordsDiv.style.position = 'fixed';
publicRecordsDiv.style.top = '40px';
publicRecordsDiv.style.right = '30px';
publicRecordsDiv.style.width = '320px';
publicRecordsDiv.style.background = '#222';
publicRecordsDiv.style.color = '#fff';
publicRecordsDiv.style.border = '3px solid #095f80';
publicRecordsDiv.style.borderRadius = '20px';
publicRecordsDiv.style.fontFamily = "'Allerta Stencil', Arial, sans-serif";
publicRecordsDiv.style.fontSize = '20px';
publicRecordsDiv.style.padding = '24px 18px 18px 18px';
publicRecordsDiv.style.boxShadow = '0 4px 16px rgba(9,95,128,0.18)';
publicRecordsDiv.style.textAlign = 'center';
publicRecordsDiv.style.letterSpacing = '2px';
publicRecordsDiv.style.zIndex = 1002;
document.body.appendChild(publicRecordsDiv);

// Funkcja do pobierania i wyświetlania publicznych rekordów
function fetchAndShowPublicRecords() {
  fetch(GLOBAL_RECORDS_URL)
    .then(res => res.json())
    .then(data => {
      if (Array.isArray(data)) {
        let html = '<b>PUBLIC RECORDS</b><br><br>';
        data.forEach((rec, idx) => {
          html += `<span style=\"font-weight:bold;\">${rec.nick}</span> - ${rec.score}<br><span style=\"font-size:15px;color:#ffe600;\">${rec.date}</span><br><br>`;
        });
        publicRecordsDiv.innerHTML = html;
      }
    })
    .catch(() => {
      publicRecordsDiv.innerHTML = 'Błąd ładowania publicznych rekordów.';
    });
}

// Wywołaj na starcie i po każdym zapisie
fetchAndShowPublicRecords();

// Funkcja do wysyłania nowego rekordu
function sendGlobalRecord(nick, score) {
  fetch(GLOBAL_RECORDS_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ nick, score })
  })
  .then(() => fetchAndShowPublicRecords());
}

// Po zakończeniu gry, jeśli wynik jest rekordem, zapytaj o nick i wyślij
function trySaveGlobalRecord(score) {
  // Sprawdź, czy pobito swój publiczny rekord
  fetch(GLOBAL_RECORDS_URL)
    .then(res => res.json())
    .then(data => {
      let myNick = localStorage.getItem('publicNick') || null;
      let myBest = 0;
      if (myNick && Array.isArray(data)) {
        let found = data.find(r => r.nick === myNick);
        if (found) myBest = found.score;
      }
      if (score > myBest) {
        let nick = myNick || prompt('Gratulacje! Podaj swój nick, aby zapisać publiczny rekord:');
        if (nick && nick.length > 0) {
          localStorage.setItem('publicNick', nick);
          sendGlobalRecord(nick, score);
        }
      }
    });
}
